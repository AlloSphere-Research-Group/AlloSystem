...
:	DistAtten<double>(nearClip, farClip, law, farBias),
	mSound(delaySize), mUseAtten(true), mDopplerType(dopplerType), mUsePerSampleProcessing(false), mSoundFocus(1)
{
...
void AudioScene::numFrames(int v){
	if(mNumFrames != v){
        ...
        mBuffer = (float*)malloc(sizeof(float)*v);
		//mBuffer.resize(v);
...
//                        #endif
			...
			else //more efficient, per buffer processing for audioscene (does not work well with doppler)
            {
                ...
                Vec3d prevpos = src.posHistory()[1] - l.pose().pos();
                
                double dist = relpos.mag();
                double prevdist = prevpos.mag();
                
                double gain = src.attenuation(dist);
                double prevgain = src.attenuation(prevdist);
                ...
                for(int i = 0; i < numFrames; i++)
                {
                    ...
                    float f = (float)i/(numFrames-1);
                    float g = f*gain + (1.f - f)*prevgain;
                    
                    mBuffer[i] = g * src.readSample(samplesAgo);
                }
                
//                #if !ALLOCORE_GENERIC_AUDIOSCENE
                spatializer->perform(io, src,relpos, numFrames, mBuffer);
//                #else
