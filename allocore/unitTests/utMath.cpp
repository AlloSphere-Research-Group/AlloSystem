...
//		a = 1. / a;				REQUIRE(a == 1./6);
	...
	// Mat
	{
		...
		//a(0,1) = a(1,0);
		#define CHECK_MATRIX(m, a,b,c, d,e,f, g,h,i)\
		REQUIRE(m(0,0)==a); REQUIRE(m(0,1)==b); REQUIRE(m(0,2)==c);\
		REQUIRE(m(1,0)==d); REQUIRE(m(1,1)==e); REQUIRE(m(1,2)==f);\
		REQUIRE(m(2,0)==g); REQUIRE(m(2,1)==h); REQUIRE(m(2,2)==i)

		a.setIdentity();	CHECK_MATRIX(a, 1,0,0, 0,1,0, 0,0,1);
		a.set(0);
		a = a.identity();	CHECK_MATRIX(a, 1,0,0, 0,1,0, 0,0,1);



		REQUIRE(a.trace() == 3);

		a += 2;		CHECK_MATRIX(a, 3,2,2, 2,3,2, 2,2,3);
		a -= 1;		CHECK_MATRIX(a, 2,1,1, 1,2,1, 1,1,2);
		a *= 2;		CHECK_MATRIX(a, 4,2,2, 2,4,2, 2,2,4);
		a /= 2;		CHECK_MATRIX(a, 2,1,1, 1,2,1, 1,1,2);

		a.setIdentity();
		a = a+2;	CHECK_MATRIX(a, 3,2,2, 2,3,2, 2,2,3);
		a = a-1;	CHECK_MATRIX(a, 2,1,1, 1,2,1, 1,1,2);
		a = a*2;	CHECK_MATRIX(a, 4,2,2, 2,4,2, 2,2,4);
		a = a/2;	CHECK_MATRIX(a, 2,1,1, 1,2,1, 1,1,2);

		a.setIdentity();
		a = 2.+a;	CHECK_MATRIX(a, 3,2,2, 2,3,2, 2,2,3);
		a = 4.-a;	CHECK_MATRIX(a, 1,2,2, 2,1,2, 2,2,1);
		a = 2.*a;	CHECK_MATRIX(a, 2,4,4, 4,2,4, 4,4,2);
	...
	// Quat
	{
		...
		// Test roundtrip Euler/quat conversion
		{
			...
			for(int i=0; i<N; ++i){
				...
				bool test = eq(a, b, eps) || eq(a, -b, eps);
				REQUIRE(test);
			}}}
		}
