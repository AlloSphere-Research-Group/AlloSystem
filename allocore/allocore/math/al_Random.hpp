#ifndef INC_AL_RANDOM_HPP
#define INC_AL_RANDOM_HPP
...
#define AL_RNG_ENVIR(initSeed)\
	al::rnd::Random<> rng(initSeed ? uint32_t(initSeed) : al::rnd::seed());\
	auto P = [&](float p=0.5){ return rng.prob(p); };\
	auto Bs= [&](float p=0.5, float v=1.){ return P(p)?v:-v; };\
	auto U = [&](){ return rng.uniform(1.f); };		/* uniform real in [ 0,1) */\
	auto Us= [&](){ return rng.uniform(1.f,-1.f); };/* uniform real in [-1,1] */\
	...
	auto T = [&](){ return (U()+U())*0.5f; };		/* triangle in [ 0,1) */\
	auto Ts= [&](){ return U()-U(); };				/* triangle in [-1,1] */\
	auto Vs= [&](){ return rng.sign(L1()); };		/* flipped triangle in [-1,1] */\
	auto V = [&](){ return Vs()*0.5f+0.5f; };		/* flipped triangle in [ 0,1] */
