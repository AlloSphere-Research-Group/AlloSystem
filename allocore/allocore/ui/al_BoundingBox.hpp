...
struct BoundingBox {
  ...
  Mesh& getTics(){
    ...
    // x tics
    for (int i = 0; i < dim.x*glUnitLength; i++){
      for (int z = 0; z < 2; z++){
        for (int y = -1; y < 2; y++){
          if (y != 0){
            float len = ticLen;
            if (i%5==0) len *= 2;
            if (i%10==0) len *= 1.5;
            tics.vertex((i/glUnitLength)-halfDim.x, y*halfDim.y, z*dim.z);
            tics.vertex((i/glUnitLength)-halfDim.x, y*halfDim.y-(len*y), z*dim.z);

            tics.vertex((i/glUnitLength)-halfDim.x, y*halfDim.y, z*dim.z);
            tics.vertex((i/glUnitLength)-halfDim.x, y*halfDim.y, z*dim.z-(((z*2)-1)*len));
          }
        }
      }
    }
    // y tics
    for (int i = 0; i < dim.y*glUnitLength; i++){
      for (int z = 0; z < 2; z++){
        for (int x = -1; x < 2; x++){
          if (x != 0){
            float len = ticLen;
            if (i%5==0) len *= 2;
            if (i%10==0) len *= 1.5;
            tics.vertex(x*halfDim.x, (i/glUnitLength)-halfDim.y, z*dim.z);
            tics.vertex(x*halfDim.x-(len*x), (i/glUnitLength)-halfDim.y, z*dim.z);

            tics.vertex(x*halfDim.x, (i/glUnitLength)-halfDim.y, z*dim.z);
            tics.vertex(x*halfDim.x, (i/glUnitLength)-halfDim.y, z*dim.z-(((z*2)-1)*len));
          }
        }
      }
    }
    // z tics
    for (int i = 0; i < dim.z*glUnitLength; i++){
      for (int z = 0; z < 2; z++){
        for (int x = -1; x < 2; x++){
          if (x != 0){
            float len = ticLen;
            if (i%5==0) len *= 2;
            if (i%10==0) len *= 1.5;
            tics.vertex(z*dim.x-halfDim.x, x*halfDim.y, (i/glUnitLength));
            tics.vertex(z*dim.x-halfDim.x, x*halfDim.y-(len*x), (i/glUnitLength));

            tics.vertex(z*dim.x-halfDim.x, x*halfDim.y, (i/glUnitLength));
            tics.vertex((z*dim.x-halfDim.x)-(((z*2)-1)*len), x*halfDim.y, (i/glUnitLength));
          }
        }
      }
    }
