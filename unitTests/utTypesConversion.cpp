...
int utTypesConversion(){
	...
	assert(uintToUnit<float>(1<<29) == 1./8);
	assert(uintToUnit<float>(1<<30) == 1./4);
	assert(uintToUnit<float>(1<<31) == 1./2);
	
	assert(uintToUnitS<float>(1<<31) == 0.0);
	assert(uintToUnitS<float>((1<<31) - (1<<30)) ==-0.5);
	assert(uintToUnitS<float>((1<<31) + (1<<30)) ==+0.5);

	//assert(unitToUInt2(0.0) == 0);	// not working exactly...
	assert(unitToUInt2(1./8) == (1<<29));
	assert(unitToUInt2(1./4) == (1<<30));
	assert(unitToUInt2(1./2) == (1<<31));
