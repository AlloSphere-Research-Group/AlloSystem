...
void delta_main_init() {
	...
	if (g_main == NULL) {
		...
		g_main->mainpq = al_pq_create(1000, 0);
		g_main->audiopq = al_pq_create(1000, 0);
		g_main->inbox = al_tube_create(10);
		g_main->outbox = al_tube_create(10);
...
void delta_main_quit() {
	if (g_main) {
		/* TODO: flush all data */
		al_pq_free(&g_main->audiopq, 0);
		al_pq_free(&g_main->mainpq, 0);
		al_tube_free(&g_main->inbox);
		al_tube_free(&g_main->outbox);
...
void delta_audio_tick(delta_samplestamp frames) {
	...
	al_sec t = g_main->elapsed / g_main->samplerate;
	
	// copy incoming tube messages to the thread-local priority queue (ensures deterministic ordering)
	al_tube_pq_transfer(g_main->inbox, g_main->audiopq, t);
	al_pq_update(g_main->audiopq, t, 0);
...
void delta_main_tick() {
	...
	*/
	al_tube_pq_transfer(g_main->outbox, g_main->mainpq, t);
	al_pq_update(g_main->mainpq, t, 0);
}	

al_sec delta_main_now() {
...
static int delta_audio_proc_unregister(al_sec t, char * args) {
	...
	process * buf = (process *)al_tube_write_head_mem(g_main->outbox);
	if (buf) {
		*buf = x;
		al_tube_write_send(g_main->outbox, t, x->freefunc);
	} else {
		return 1;
	}
	return 0;
...
int delta_audio_proc_init(process x, al_msg_func procmsg, al_msg_func freemsg) {
	process * buf = (process *)al_tube_write_head_mem(g_main->inbox);	
	*(process *)x->msg.mem = x;
	x->msg.func = procmsg;
	x->freefunc = freemsg;
	x->id = 0;
	if (buf) {
		*buf = x;
		al_tube_write_send(g_main->inbox, g_main->mainpq->now, delta_audio_proc_register);
	} else {
		return 1;
	}
	return 0;
...
int delta_audio_proc_gc(process * ptr) {
	...
	if (ptr && *ptr) {
		...
		if (buf) {
			*buf = x;
			al_tube_write_send(g_main->inbox, g_main->mainpq->now, delta_audio_proc_unregister);
			return 0;
		} else {
			/* TODO: reschedule */
