...
for libdir in $LIB_DIRS; do
	cd $libdir
	${EM_DIR}emmake make install $* PLATFORM=em ARCH=none SDL_VERSION=2 WINDOW_BINDING=SDL AUDIO_BINDING=SDL USE_HID=0 USE_MIDI=0 USE_ZEROCONF=0 NO_AUDIO_IO=1 BUILD_DIR=$BUILD_DIR DESTDIR=$BUILD_DIR
	cd $ALLO_DIR
