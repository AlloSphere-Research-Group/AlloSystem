...
for libdir in $LIB_DIRS; do
	cd $libdir
	emmake make install $* PLATFORM=em ARCH=none WINDOW_BINDING=SDL AUDIO_BINDING=SDL USE_HID=0 USE_MIDI=0 USE_ZEROCONF=0 USE_ASSET=0 USE_FONT=0 NO_AUDIO_IO=1 BUILD_DIR=$BUILD_DIR DESTDIR=$BUILD_DIR
	cd $ALLO_DIR
